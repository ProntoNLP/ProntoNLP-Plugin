...
description: "Trigger when the user asks about a sector, industry, or theme across many companies — not a single company. Examples: 'analyze the tech sector', 'healthcare industry report', 'what's happening in financials', 'energy sector outlook', 'which companies in semiconductors are leading', 'sentiment in consumer discretionary', 'top movers in industrials', 'what themes are rising in biotech', 'AI across the information technology sector', 'which sectors are most bullish', 'real estate sector trends', 'which companies in fintech talk about interest rates most'. Do NOT trigger for a single named company — use pronto-company-intelligence instead. When in doubt — if the user names a sector or industry rather than one company — use this skill."
metadata:
  author: ProntoNLP
  version: 1.0.0
...
- Design the layout to be clean and readable — cards, tables, badges, section headers
...
**Batch 5** — render full inline HTML report with all charts.
...
Output all charts as inline HTML — no file writing. Load Chart.js once near the top:
